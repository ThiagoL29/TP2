      ...
      for (int i = 0; i<tam_grid; i++) {
        for (int j= 0; j<tam_grid; j++) {
          ...
          if(i+1<tam_grid && (matriz[i+1][j]== -3 || matriz[i+1][j]>0) && matriz[i][j]!=-2){
            if(matriz[i+1][j]== -3) {matriz[i][j] = 1;  contador++; break;}
          ...
          if(j-1 >=0 && (matriz[i][j-1]== -3 || matriz[i][j-1]>0)&& matriz[i][j]!=-2){
            if(matriz[i][j-1]== -3) {matriz[i][j] = 1; contador++; break;}
          ...
          if(local_adj == 1 && i>=0){matriz[i][j] = matriz[i-1][j]+1; contador++;}
          if(local_adj == 2 && i<tam_grid){matriz[i][j] = matriz[i+1][j]+1; contador++;}
          if(local_adj == 3 && j>=0 ){matriz[i][j] =matriz[i][j-1]+1;  contador++;}
          if(local_adj == 4 && j<tam_grid ){matriz[i][j] = matriz[i][j+1]+1; contador++;}
          
          if(contador>= (tam_grid * tam_grid) ){
