 ...
 void simulacao(int[][] matriz){
  ...
  while(matriz[asRx][asRy] != matriz[asDx][asDy]){
    ...
    if(asRx-1>=0 && matriz[asRx-1][asRy] <menor_caminho && matriz[asRx-1][asRy] !=0){
      menor_caminho = matriz[asRx-1][asRy];
      direcao_caminho = 1;
      
  
    }
     if(asRx+1<tam_grid && matriz[asRx+1][asRy] <menor_caminho && matriz[asRx+1][asRy]!=0){
      menor_caminho = matriz[asRx+1][asRy];
      direcao_caminho = 2;
    
    }
     if(asRy-1>=0 && matriz[asRx][asRy-1] <menor_caminho &&matriz[asRx][asRy-1]!= 0){
      menor_caminho = matriz[asRx][asRy-1];
      direcao_caminho = 3;
    
    }
     if(asRy+1<tam_grid && matriz[asRx][asRy+1] <menor_caminho && matriz[asRx][asRy+1] != 0){
      menor_caminho = matriz[asRx][asRy+1];
      direcao_caminho = 4;
